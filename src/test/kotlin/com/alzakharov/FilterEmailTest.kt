...
class FilterEmailTest {
    ...
    fun testValidEmailAddresses() {
        ...
        assertTrue(email.validate { ValidationFilters.email(it) }, "valid email should return 'true'")
    }

    @Test
    fun testInvalidEmailAddresses() {
        var email = "kotlin-validators@alzakharov"
        assertFalse(email.validate { ValidationFilters.email(it) }, "invalid email should return 'false'")
